...
function ContactItem({ icon, label, children }: ContactItemProps) {
  return (
    <div className="flex gap-4">
      <div className="w-10 h-10 rounded-lg bg-cta/10 text-cta flex items-center justify-center shrink-0">
        {icon}
      </div>
      <div>
        <p className="text-sm font-medium text-muted">{label}</p>
        <div className="mt-0.5 text-navy font-medium">{children}</div>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    <Card padding="lg">
      ...
      <div className="space-y-6">
        ...
        >
          <span className="text-sm">{BUSINESS_HOURS}</span>
        </ContactItem>
